...
class SeedingPerformanceWriter final : public WriterT<SeedContainer> {
 public:
  struct Config {
    /// Input truth particles collection.
    std::string inputParticles;
    /// Input (found) seeds collection.
    std::string inputSeeds;
    /// Output directory.
    std::string outputDir;
    /// Output filename.
    std::string outputFilename = "performance_seeding.root";
 ...
 private:
  ProcessCode writeT(const AlgorithmContext& ctx,
                     const SeedContainer& seeds) final override;
