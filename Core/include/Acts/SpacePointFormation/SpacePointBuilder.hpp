...
///
  ...
  protected:
    ...
    std::pair<Acts::Vector3, Acts::Vector3> endsOfStrip(const Acts::GeometryContext& gctx, const Measurement& measurement) const;


    // Acts::SourceLink getSourceLink( const Measurement& meas) const;

    // Acts::Vector2 globalCov(const Acts::GeometryContext& gctx, const Acts::GeometryIdentifier& geoId,const Acts::Vector2& localPos, const Acts::SymMatrix2& localCov) const;
      
    // double getLocVar(const Measurement& meas) const;
  
    // Acts::Vector2 getGlobalVars( const Acts::GeometryContext& gctx, const Measurement& meas_front, const Measurement& meas_back, const double theta) const;
